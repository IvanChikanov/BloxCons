class HTMLFather
{
    ...
    {
        let req = await fetch(HTMLFather.editPath(pathPeacesArr));
    ...
    {
       await fetch(HTMLFather.editPath(pathPeacesArr));
    }
    ...
    {
        return await HTMLFather.getJSON(["pages","get_empty_unit", cell_id]);
    }
    static createFileloaderElement(extensions, callBack)
    {
        let fileloader = HTMLFather.create("INPUT");
        fileloader.type = "file";
        fileloader.setAttribute("accept", extensions);
        HTMLFather.addStyles(["fullScreenSize", "borderBox"],[fileloader]);
    ...
    { 
        let saveIm = unit.imageId? HTMLFather.updateImage : HTMLFather.createImage;
